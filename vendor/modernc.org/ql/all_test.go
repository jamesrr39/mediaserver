...
func TestIssue219v1(t *testing.T) { testIssue219(t, "ql") }
func TestIssue219v2(t *testing.T) { testIssue219(t, "ql2") }

func testIssue219(t *testing.T, drv string) {
	if testing.Short() {
		t.Skip("skipping test in short mode.")
	}
	...
	defer sdb.Close()
	tx, err := sdb.BeginTx(context.Background(), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer mdb.Close()
	if tx, err = mdb.BeginTx(context.Background(), nil); err != nil {
		t.Fatal(err)
	}
